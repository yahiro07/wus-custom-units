...
export function ThreeOhGen(): NoteGenerator {
  let noteSet: GeneralisedParameter<FullNote[]> = genericParameter("note set", [
    "C1",
  ]);
  ...
  function createPattern(): Pattern {
    ...
    for (let i = 0; i < 16; i++) {
      const chance =
        density *
        (i % 4 === 0 ? 0.6 : i % 3 === 0 ? 0.5 : i % 2 === 0 ? 0.3 : 0.1);
...
export function NineOhGen() {
  function createPatterns(full: boolean = false) {
    ...
    const hatMode: string = choose([
      "offbeats",
      "closed",
      full ? "offbeats" : "none",
    ]);
    const snareMode: string = choose([
      "backbeat",
      "skip",
      full ? "backbeat" : "none",
    ]);
