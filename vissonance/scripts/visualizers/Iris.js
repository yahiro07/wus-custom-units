    ...
    render: function () {
      ...
      if (group) {
        for (var i = 0; i < visualArray.length / 2; i++) {
          ...
          group.children[i * 2].geometry.attributes.position.array[7] =
            visualArray[i] / 2 + (65 + loudness / 1.5);
          group.children[i * 2].geometry.attributes.position.array[10] =
            visualArray[i] / 2 + (65 + loudness / 1.5);
          group.children[i * 2].geometry.attributes.position.needsUpdate = true;

          group.children[i * 2 + 1].geometry.attributes.position.array[7] =
            visualArray[i] / 2 + (65 + loudness / 1.5);
          group.children[i * 2 + 1].geometry.attributes.position.array[10] =
            visualArray[i] / 2 + (65 + loudness / 1.5);
          group.children[i * 2 + 1].geometry.attributes.position.needsUpdate =
            true;
        }
      }
    },
