function Siphon() {
  ...
  var siphon = {
    ...
    render: function () {
      ...
      if (group) {
        group.scale.x = 1 - Math.min(loudness / 255, 0.8);
        group.scale.y = 1 - Math.min(loudness / 255, 0.8);

        for (var c = 0; c < group.children.length; c++) {
          group.children[c].position.z +=
            loudness <= 1
              ? 0
        ...
        for (var i = 0; i < visualArray.length; i++) {
          ...
          scaleGroupVectorLength(
            currentBand,
            v / 2 - 3 - (numBars * 3 + i * 3) + numBars * 3 + numBars * 6,
            -visualArray[i] / 3.5 - loudness / 7,
          );
          group.children[currentBand].geometry.attributes.position.needsUpdate =
            true;
        }
  ...
  function scaleGroupVectorLength(groupC, groupI, length) {
    ...
    var scalar =
      (length + cylRadius) / v3.distanceTo(new THREE.Vector3(0, 0, 0));
